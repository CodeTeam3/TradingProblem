...
public class Account {
	...
	public void updateAccountBalances() {
		...
		for(Ticker t : tickers.getTickers()) {
			if(t.getEquity() > 0) {
				stockbalanceTEMP += t.getPrice() * t.getShares();
			}
			else if(0 > t.getEquity()) {
				shortholdingTEMP += t.getEquity();
				shortToCoverTEMP += t.getPrice() * t.getShares();
			}
		}
	...
	public void buyShares(String symbol, double sharesToBuy) {
		...
		double cashValue = t.getPrice() * sharesToBuy;	//	Get's the cash value of the new position then checks if we have enough cash before ordering
		//	TO BUY
	...
	public void sellShares(String symbol, double sharesToSell) {
		...
		if(newShares >= 0) {
			cashbalance +=  t.getPrice() * sharesToSell; 
			t.Sell(sharesToSell);
		}
	...
	public void printPositions() {
		...
		if(stockbalance != 0 || shortToCover != 0) {
			...
			System.out.println("    SYM   Shares   Price   Avg Price    Market       Cost     Profit/Loss   %");
			for(Ticker t : tickers.getTickers()) {
				if(t.getShares() == 0) {
					continue;
				}
				System.out.println(c + ". [" + 		// 	Symbol - Shares - Price - Avg Price - Total - Cost - %change
						...
						"| $" + String.format("%7s", String.format("%,.2f",t.getPrice())) + 
						"| $" + String.format("%7s", String.format("%,.2f", t.getAveragePrice())) +
						"| $" + String.format("%10s", String.format("%,.2f", t.getShares() * t.getPrice())) + 
